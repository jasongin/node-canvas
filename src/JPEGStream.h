...
empty_closure_output_buffer(j_compress_ptr cinfo){
  ...
  Napi::Value buf = Napi::Buffer::New(env, (char *)dest->buffer, dest->bufsize);
...
term_closure_destination(j_compress_ptr cinfo){
  ...
  /* emit remaining data */
  Napi::Value buf = Napi::Buffer::New(env, (char *)dest->buffer, dest->bufsize - dest->pub.free_in_buffer);
